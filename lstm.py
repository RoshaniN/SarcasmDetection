...
tokenizer.fit_on_texts(X_train_or['headline'].values)
word_index = tokenizer.word_index
print('Found %s unique tokens.' % len(word_index))

X_train = tokenizer.texts_to_sequences(X_train_or['headline'].values) # Every word got a new number
# print(X_train[1])
...
X_test = tokenizer.texts_to_sequences(X_test_or['headline'].values) # Every word got a new number
# print(X_test[1])
...
X_valid = tokenizer.texts_to_sequences(X_valid_or['headline'].values) # Every word got a new number
# print(X_valid[1])
...
Y_train = pd.get_dummies(X_train_or['y']).values
print('Shape of train label tensor:', Y_train.shape)

Y_test = pd.get_dummies(X_test_or['y']).values
print('Shape of test label tensor:', Y_test.shape)

Y_valid = pd.get_dummies(X_valid_or['y']).values
